...
const InvoiceReports = () => {
  ...
  // Table headers for dynamic rendering
  const tableHeaders = [
    'Name',
    'Customer Code',
    'Account Number',
  ...
  const fetchReports = async (page, limit, search = '', start = '', end = '', atoll = '', island = '') => {
    setLoading(true);
    try {
      console.log('Fetching with params:', { page, limit, search, start, end, atoll, island });
      let url = `${API_URL}/invoiceReports?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}&format=json`;
  ...
  const handleFilter = () => {
    ...
    setAppliedStartDate(formatDate(startDate));
    setAppliedEndDate(formatDate(endDate));
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      console.log('Downloading CSV with params:', { searchTerm, appliedStartDate, appliedEndDate, appliedAtoll, appliedIsland });
      let url = new URL(`${API_URL}/invoiceReports`, window.location.origin);
